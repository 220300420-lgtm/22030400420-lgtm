...
(function () {
  ...
  // ============================================================
  const GEMINI_API_KEY = 'AIzaSyC2xWwWauShY8bTAR8mLlUq8reDthTIxhE';
...
`;
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      console.log('CenBot Gemini response:', JSON.stringify(data));

      if (data.error) throw new Error(data.error.message);
      if (!data.candidates || !data.candidates[0]) throw new Error('Sin respuesta de Gemini');
      ...
      hideTyping();
      appendMessage('bot', `😅 Error: ${err.message}. Revisa la consola del navegador (F12) para más detalles.`);
      console.error('CenBot error completo:', err);
    }
