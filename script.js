...
form.addEventListener('submit', e => {
  e.preventDefault(); clearErrors(); let ok = true;
  const v = id => document.getElementById(id).value.trim();
  if (!v('empresa')) { showError('empresa','Ingresa el nombre de tu empresa.'); ok=false; }
  if (!v('responsable')) { showError('responsable','Ingresa tu nombre.'); ok=false; }
  if (!v('email')) { showError('email','Ingresa tu correo.'); ok=false; }
  else if (!validEmail(v('email'))) { showError('email','Correo inválido.'); ok=false; }
  if (!document.getElementById('tipo').value) { showError('tipo','Selecciona el tipo de negocio.'); ok=false; }
  if (!document.getElementById('empleados').value) { showError('empleados','Selecciona el número de empleados.'); ok=false; }
  if (!v('problema')) { showError('problema','Describe tu reto principal.'); ok=false; }
  if (ok) {
    const btn = form.querySelector('[type="submit"]');
    btn.textContent = 'Enviando…'; btn.disabled = true;
    setTimeout(() => { form.classList.add('hidden'); successDiv.classList.add('show'); }, 900);
  }
});
